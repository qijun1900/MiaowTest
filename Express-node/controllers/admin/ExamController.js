...
const ExamController = {
  ...
  updateInfo: async (req, res) => {
    const cover = req.file ? `/examcoveruploads/${req.file.filename}` : "";
    const { name, code, category, year, isPublish, creator, day, _id } =
      req.body;
    const parsedCategory = JSON.parse(category).map(Number);
  ...
  UpdateSelectQuestion: async (req, res) => {
    const { _id } = req.query;
    const { stem, options, isPublish, analysis, isAIanswer, isMultiple } =
      req.body;
  ...
  UpdateOneQuestionTitleState: async (req, res) => {
    ...
    await ExamService.UpdateOneQuestionTitleState({
      _id,
      examId,
      isPublish: Number(state),
    });
    res.status(200).send({
      code: 200,
      ActionType: "OK",
  ...
  AddOneQuestion: async (req, res) => {
    ...
    await ExamService.AddOneQuestion({
      _id,
      examId,
      category,
      QuestionTitleId,
  ...
  AddManyQuestion: async (req, res) => {
    ...
    await ExamService.AddManyQuestion({
      _ids,
      examId,
      category,
      QuestionTitleId,
  ...
  getCheckQusetionList: async (req, res) => {
    ...
    const result = await ExamService.getCheckQusetionList({
      examId,
      QuestionTitleId,
    });
    res.status(200).send({
  ...
  RemoveUserQuestionList: async (req, res) => {
    ...
    await ExamService.RemoveUserQuestionList({
      chooseInfo,
      examId,
      QuestionTitleId,
    });
