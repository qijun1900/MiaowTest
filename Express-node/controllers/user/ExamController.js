...
const ExamController = {
  ...
  userDeleteQuestion: async (req, res) => {
    try {
      ...
      const result = await ExamService.userDeleteQuestion({
        uid,
        questionId,
        bankId,
      });
  ...
  userAddWrongQuestion: async (req, res) => {
    try {
      ...
      const result = await ExamService.useraddwrongquestion({
        uid,
        questionId,
        examId,
        Type,
  ...
  userDeleteWrongQuestion: async (req, res) => {
    try {
      ...
      const result = await ExamService.userDeleteWrongQuestion({
        uid,
        questionId,
      });
      res.status(200).send({
  ...
  userAddFavoriteQuestion: async (req, res) => {
    try {
      const { uid } = req.user; //获取用户openid
      const { questionId, examId, Type } = req.body; // 从请求体中获取questionId,examId,Type
      const result = await ExamService.useraddfavoritequestion({
        uid,
        questionId,
        examId,
        Type,
  ...
  userDeleteFavoriteQuestion: async (req, res) => {
    try {
      ...
      const result = await ExamService.userDeleteFavoriteQuestion({
        uid,
        questionId,
      });
      res.status(200).send({
      ...
      console.error(
        "Error fetching userDeleteFavoriteQuestion details:",
        error,
      ); // 处理错误
    }
  },
  getUserFavoriteQuestionList: async (req, res) => {
    try {
      ...
      console.error(
        "Error fetching getUserFavoriteQuestionList details:",
        error,
      ); // 处理错误
    }
  },
  checkFavoriteQuestion: async (req, res) => {
    try {
      const { uid } = req.user; //获取用户openid
      const { questionId } = req.body; // 从请求体中获取questionId
      const result = await ExamService.checkFavoriteQuestion({
        uid,
        questionId,
      });
      res.status(200).send({
  ...
  userPracticeFavoriteQuestion: async (req, res) => {
    try {
      ...
      console.error(
        "Error fetching userPracticeFavoriteQuestion details:",
        error,
      ); // 处理错误
    }
  },
  ...
  getExamSubjectTitleUrl: async (req, res) => {
    try {
      ...
      const result = await ExamService.getExamSubjectTitleUrl({
        examId,
        titleid,
      });
      res.status(200).send({
