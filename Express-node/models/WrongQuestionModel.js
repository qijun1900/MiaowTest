...
const WrongQuestionSchema = new mongoose.Schema({
  ...
  Type: {
    //// 1-选择题 2-填空题 3-判断题 4-简答题
    type: Number,
    enum: [1, 2, 3, 4],
    required: true,
  },
  questionSource: {
    // 题目来源：system-系统题库 user-用户自建
    type: String,
    enum: ["system", "user"],
    default: "user",
  },
  ...
  // 复习状态
  status: {
    // 状态：0-new（新题） 1-reviewing（复习中） 2-mastered（已掌握）
    type: Number,
    enum: [0, 1, 2],
    default: 0,
  },
...
// 实例方法：添加复习记录
WrongQuestionSchema.methods.addReviewRecord = function (
  isCorrect,
  timeSpent = 0,
) {
  this.reviewCount += 1;
...
const WrongQuestionModel = mongoose.model(
  "wrong_question",
  WrongQuestionSchema,
);
