...
const UserService = {
  ...
  getExamFavorites: async (examId, uid) => {
    try {
      ...
      // 检查用户是否收藏了该考试
      const isFavorited =
        user.favoriteExams && user.favoriteExams.includes(examId);
  ...
  userFeedback: async ({ uid, type, content, contactInfo, relatedId }) => {
    try {
      // 创建新的反馈记录
  ...
  getDotDates: async ({ uid }) => {
    try {
      ...
      // todos_content不为空的日期
      const filteredDates = todosList.filter(
        (todo) => todo.todos_content.length > 0,
      );
      // 提取日期
  ...
  editTodo: async ({ uid, fulldate, todoId, todoForm }) => {
    try {
      ...
      // 更新待办事项内容
      userTodos.todos_content[todoIndex] = {
        ...userTodos.todos_content[todoIndex],
        ...todoForm,
      };
