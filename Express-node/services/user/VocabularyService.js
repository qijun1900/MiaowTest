...
const VocabularyService = {
  ...
  setWordRember: async ({
    ...
    try {
      ...
      if (existing) {
        await ConsumerWordModel.updateOne(
          { uid },
          {
  ...
  checkWordRember: async ({ uid }) => {
    try {
      const userSettings = await ConsumerWordModel.findOne(
        { uid },
        { "settings.currentBook_id": 1, "settings.dailyGoal": 1 },
      );
      return {
          ...
          { $limit: pageSize }, // 4. 限制
          {
            $project: {
              ...
              phonetic: {
                $concat: [
                  "/",
                  { $ifNull: ["$content.word.content.usphone", ""] },
                  "/",
                ],
              },
              // 获取数组第一个元素的字段
              ...
              firstSentence: {
                $arrayElemAt: ["$content.word.content.sentence.sentences", 0],
              },
            },
          },
          {
