...
export async function CheckQuestionTitle(_id, examId) {
  // 检查题目标题是否还有题目
  try {
    const response = await axios.post("/adminapi/questionTitle/checkQuestionTitle", {
      _id,
      examId,
    });
    return response.data;
...
export async function DeleteOneQuestionTitle(_id, examId) {
  // 删除题目题型标题
  try {
    const response = await axios.post("/adminapi/questionTitle/deleteOneQuestionTitle", {
      _id,
      examId,
    });
    return response.data;
...
export async function DeleteManyQuestionTitle(_ids, examId) {
  // 删除多个题目标题
  try {
    const response = await axios.post("/adminapi/questionTitle/deleteManyQuestionTitle", {
      _ids,
      examId,
    });
    return response.data;
...
export async function UpdateQuestionTitleOneState({ _id, examId, state }) {
  // 更新单个题目标题状态
  try {
    const response = await axios.post("/adminapi/questionTitle/updateQuestionOneTitleState", {
      _id,
      examId,
      state,
    });
