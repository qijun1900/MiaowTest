...
export async function UpdateManyQuestion(data, questionType) {
  ...
  try {
    ...
    const response = await axios.post("/adminapi/exam/update/ManyPublishQuestionstate", {
      Ids,
      questionType,
    });
    return response.data;
...
export async function DeleteOneQuestion(_id, questionType) {
  //删除一条题目
  try {
    const response = await axios.post("/adminapi/exam/question/deleteOneQuestion", {
      _id,
      questionType,
    });
    return response.data;
...
export async function DeleteManyQuestion(data, questionType) {
  ...
  try {
    const Ids = data.map((ids) => ids._id);
    const response = await axios.post("/adminapi/exam/question/deleteManyQuestion", {
      Ids,
      questionType,
    });
    return response.data;
