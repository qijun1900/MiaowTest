  ...
  >
    <div v-if="previewData" class="preview-content">
      ...
      <div v-if="previewData.Type === 1" class="options-container">
        ...
        <div class="correct-answer">
          <el-tag type="success" size="large"
            >正确答案: {{ formatSelectAnswer(previewData.options) }}</el-tag
          >
        </div>
      </div>
      ...
      <div v-else-if="previewData.Type === 3" class="judge-container">
        <el-tag
          :type="previewData.answer == 1 ? 'success' : 'danger'"
          size="large"
          class="judge-tag"
        >
      ...
      <div v-if="previewData.analysis" v-html="previewData.analysis" class="analysis-content"></div>
      <div v-else class="analysis-content">暂无解析</div>

      <div class="info-footer">
        <el-tag type="info" class="info-tag"
          >更新时间: {{ formatTime.getTime2(previewData.createdTime) }}</el-tag
        >
    ...
    <template #footer>
      <el-button type="primary" @click="dialogConfirmClose" class="close-btn"> 关闭 </el-button>
    </template>
  </el-dialog>
</template>
<script setup>
