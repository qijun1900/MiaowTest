...
async function AutoAnalysisQuestion(prompt) {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      throw new Error(
        `Request failed: request_id=${response.headers["request_id"]}, code=${response.status}, message=${response.data.message}`,
      );
    }
  } catch (error) {
    ...
    if (error.response && error.response.status === 429) {
      ...
      error.message =
        "API rate limit exceeded. Please check your usage limits.";
    }
