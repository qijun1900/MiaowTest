...
const LLMService = {
  ...
  updateModel: async ({
    ...
    return await LLMModel.updateOne(
      { _id },
      { modelName, modelValue, description, creator, editTime },
    );
  },
  ...
  getQuestionAnalysis: async (message, questionType, _id) => {
    try {
      // 使用重试机制包装API调用
      const data = await withRetry.withRetry(
        async () => {
          return await AutoAnalysisQuestionmodelapp.AutoAnalysisQuestion(
            message,
          );
        },
        5,
