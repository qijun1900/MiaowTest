...
const ExamService = {
  ...
  getexamList: async ({ page, size }) => {
    ...
    const [data, total] = await Promise.all([
      ExamModel.find({}).skip(skip).limit(size).sort({ createdTime: -1 }),
  ...
  getSelectQuestionList: async ({ page, size, examId, questionType }) => {
    ...
    const [data, total] = await Promise.all([
      model.find({ examId }).skip(skip).limit(size),
      model.countDocuments({ examId }),
    ]);
    return { data, total };
  },
  AddBlankQuestion: async ({
    examId,
    stem,
    options,
    isPublish,
  ...
  UpdateBatchQuestion: async ({ Ids, questionType }) => {
    ...
    return (
      modelMap[questionType]?.updateMany(
        { _id: { $in: Ids } },
  ...
  UpdateSelectQuestion: async ({
    ...
    // 等待更新操作完成
    const updateResult = await ExamSelectModel.updateOne(
      { _id },
      {
        stem,
        options,
    ...
    // 执行删除操作
    const deleteAIResult = await AianalysisModel.deleteMany({
      questionId: _id,
    });
  ...
  UpdateBlankQuestion: async ({
    ...
    const updateResult = await ExamBlankModel.updateOne(
      { _id },
      {
        stem,
        options,
  ...
  UpdateJudgeQuestion: async ({
    ...
    const updateResult = await ExamJudgeModel.updateOne(
      { _id },
      {
        stem,
        answer,
  ...
  UpdateShortQuestionList: async ({
    ...
    const updateResult = await ExamShortModel.updateOne(
      { _id },
      {
        stem,
        content,
  ...
  CheckQuestionTitle: async ({ examId, _id }) => {
    ...
    if (
      userExam &&
      userExam.questionTitle &&
      userExam.questionTitle.length > 0
    ) {
      const hasQuestion =
        userExam.questionTitle[0].questionIdS &&
        userExam.questionTitle[0].questionIdS.length > 0;
      return { hasQuestion };
    }
  ...
  getAddQusetionList: async ({ category, examId }) => {
    ...
    return (
      model.find(
        { examId: examId, isAddUserList: 0, isPublish: 1 },
        { _id: 1, stem: 1, isAddUserList: 1 },
      ) || null
    );
  },
  AddOneQuestion: async ({ _id, examId, category, QuestionTitleId }) => {
    ...
    const [updateResult, insertResult] = await Promise.all([
      model.updateOne({ _id }, { $set: { isAddUserList: 1 } }),
  ...
  AddManyQuestion: async ({ _ids, examId, category, QuestionTitleId }) => {
    ...
    const model = modelMap[category];
    const [updateResult, insertResult] = await Promise.all([
      model.updateMany({ _id: { $in: _ids } }, { $set: { isAddUserList: 1 } }),
      UserExamModel.updateOne(
        { examId, "questionTitle._id": QuestionTitleId },
        {
          $push: {
            "questionTitle.$.questionIdS": {
              $each: _ids.map((_id) => ({ _id, category })),
            },
          },
        },
      ),
  ...
  RemoveUserQuestionList: async ({ chooseInfo, examId, QuestionTitleId }) => {
    ...
    const updatePromises = chooseInfo.map(({ _id, category }) => {
      // 更新题目状态
      const model = modelMap[category];
      return model.updateOne({ _id }, { isAddUserList: 0 });
    });
    const deletePromises = chooseInfo.map(({ _id }) => {
      // 删除题目ID
      return UserExamModel.updateOne(
        { examId, "questionTitle._id": QuestionTitleId },
  ...
  batchPublishedUserQuestionsList: async ({
    examId,
    Type,
    titleId,
    questionId,
