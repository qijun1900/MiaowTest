<template>
    <view class="date">
        ...
        <swiper
            :style="{
                height:
                    (retract ? 2 * 80 : (week_list.length + 1) * 80) + 'rpx',
            }"
            :current="current"
            circular
            @change="change_date"
        >
            <swiper-item>
                ...
                <view @click="open" class="retract icon"
                    ><text
                        class="iconfont next icon-fanhui"
                        :class="[retract ? '' : 'retract_icon']"
                /></view>
            </swiper-item>
            <swiper-item>
                <view
...
export default {
    ...
    computed: {
        week_list_prev_co() {
            return this.retract
                ? this.week_list_prev_week
                : this.week_list_prev;
        },
        week_list_next_co() {
            return this.retract
                ? this.week_list_next_week
                : this.week_list_next;
        },
    },
    mounted() {
    ...
    methods: {
        ...
        change_week() {
            ...
            if (this.to_week_index == 0) {
                ...
                // if(){
                let next_day =
                    this.week_list_prev[this.week_list_prev.length - 1][6].day;
        ...
        change_date(e) {
            ...
            if (
                primary_current - current == -1 ||
                primary_current - current == 2
            ) {
                if (this.retract) {
                ...
                list.push(
                    item.map((vo, key) => {
                        if (
                            ...
                            this.dot_list.indexOf(
                                vo.date.replace(/-(\d)(?!\d)/g, "-0$1"),
                            ) > -1
                        ) {
                            vo.dot = true;
                        } else {
        ...
        get_date(value = "", type = "same") {
            ...
            for (
                let i = prev_date_days - start_week + 1;
                i <= prev_date_days;
                i++
            ) {
                date_arrs.push({
