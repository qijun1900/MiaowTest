    ...
    <view class="container">
        ...
        <template v-else>
            ...
            <view class="question-form">
                <view
                    v-if="selectedQuestionTypeValue === 1"
                    :key="1"
                    class="form-item fade-slide"
                >
                    <AddSelect
                        ref="addSelectRef"
                ...
                <view
                    v-if="selectedQuestionTypeValue === 2"
                    :key="2"
                    class="form-item fade-slide"
                >
                    <AddBlank
                ...
                <view
                    v-if="selectedQuestionTypeValue === 3"
                    :key="3"
                    class="form-item fade-slide"
                >
                    <AddJudge
                ...
                <view
                    v-if="selectedQuestionTypeValue === 4"
                    :key="4"
                    class="form-item fade-slide"
                >
                    <AddShort
    ...
    try {
        ...
        if (res.code === 200 && res.data) {
            ...
            selectedDifficulty.value =
                difficultyMap[data.difficulty] || difficultyMap.medium;
        } else {
            uni.showToast({
...
const handleQuestionSubmit = async (questionData) => {
    try {
        ...
        if (res.code === 200) {
            ...
            uni.showToast({
                title:
                    res.message || (isEditMode.value ? "更新失败" : "添加失败"),
                icon: "none",
            });
        }
    } catch (error) {
        console.error(
            isEditMode.value ? "更新错题失败:" : "提交错题失败:",
            error,
        );
        uni.showToast({
            title: "提交失败，请重试",
