    ...
    <view>
            ...
            >
                ...
                >
                    <view class="question-container">
                        <SelectQuestion
                            v-if="
                                questionStore.UserChooseQuestion[itemIndex]
                                    ?.Type === 1
                            "
                            :question="
                                questionStore.UserChooseQuestion[itemIndex]
        ...
        <dragButton
            ...
            :content="[
                ...
                {
                    text: '收藏',
                    iconType: currenIsFavorited ? 'star-filled' : 'star',
                    value: 2,
                },
            ]"
                ...
                <view class="note-popup-container">
                    ...
                    <view class="note-info" v-if="lastSavedTime">
                        <text class="note-info-text"
                            >上次保存:
                            {{ formatTime.getTime2(lastSavedTime) }}</text
                        >
                    </view>
...
const bankInfo = ref({ isUserBank: false }); // 提供默认值
...
const handleMenuClick = async (item) => {
    ...
    if (
        ...
        // 取消收藏当前问题
        const res = await deleteFavoriteQuestionAPI(currentQuestionId.value);
        if (res.code === 200) {
    ...
    if (
        ...
        // 加载已有的笔记(检测是否有笔记)
        const res = await getPracticeNoteAPI(
            bankInfo.value.isUserBank,
            currentQuestionId.value,
        );
...
const updateCurrentQuestionId = () => {
    ...
    if (
        ...
        questionStore.UserChooseQuestion[currentQuestionIndex]
    ) {
        currentQuestionId.value =
            questionStore.UserChooseQuestion[currentQuestionIndex]._id; // 更新当前问题ID
        currenQuestionType.value =
            questionStore.UserChooseQuestion[currentQuestionIndex].Type; // 更新当前问题的类型
    }
};
...
const isEndQuestion = computed(() => {
    const currentQuestionIndex = visibleIndexes.value[currentIndex.value];
    if (
        currentQuestionIndex === questionStore.UserChooseQuestion.length - 1 &&
        currentMode.value === 0
    ) {
        return true;
    } else {
...
onMounted(() => {
    ...
    if (questionCount >= 3) {
        ...
        visibleIndexes.value = Array.from(
