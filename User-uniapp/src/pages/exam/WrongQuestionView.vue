                ...
                <view class="question-header">
                    <view class="question-info">
                        ...
                        <text class="question-type">{{
                            formatInfo.getQuestionTypeText(question.Type)
                        }}</text>
                    </view>
                    ...
                    >
                        <view class="section-title">
                            ...
                            <text class="title-text">{{
                                question.isMultiple === 1 ? "多选题" : "单选题"
                            }}</text>
                        </view>
                        <SelectOptionsPreview
                            :options="question.options"
                            :userWrongAnswer="
                                getUserObjectiveAnswerText(question._id)
                            "
                            :showTitle="false"
                        />
                    </view>
                        ...
                        <view class="user-answer-section">
                            ...
                            <view class="answer-content">
                                <view
                                    v-for="(
                                        answer, answerIndex
                                    ) in getUserSubjectiveAnswer(question._id)"
                                    :key="answerIndex"
                                    class="blank-answer-item"
                                >
                                    <text class="blank-index"
                                        >空{{ answerIndex + 1 }}：</text
                                    >
                                    <text class="blank-answer">{{
                                        answer || "未填写"
                                    }}</text>
                                </view>
                            </view>
...
const filteredQuestions = computed(() => {
    ...
    return wrongQuestions.value.filter(
        (question) => question.Type === currentTypeIndex.value,
    );
});
...
const addedToBookCount = computed(() => {
    // 加入错题本的题目数量
    ...
    try {
        ...
        currentQuestions.forEach((question) => {
            const isObjectiveWrong =
                objectiveAnswerStore.getIsAnswerCorrect(question._id) === false;
            const isSubjectiveWrong =
                subjectiveAnswerStore.getUserSelfEvaluation(question._id) ===
                false;

            if (isObjectiveWrong || isSubjectiveWrong) {
                // 客观题或主观题有错误
                wrongQuestionsData.push({
                    ...question,
                    userAnswer:
                        objectiveAnswerStore.getUserAnswer(question._id) ||
                        subjectiveAnswerStore.getUserAnswer(question._id),
                    isCorrect: false,
...
const toggleWrongBook = async (question) => {
    ...
    try {
        ...
        if (isCurrentlyInBook) {
            ...
            // 调用加入错题本API
            const res = await addWrongQuestionAPI(
                question._id,
                question.examId,
                question.Type,
            );
